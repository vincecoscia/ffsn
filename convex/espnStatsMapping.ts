...
// Helper function to transform stat IDs to readable names
export const transformStats = (stats: Record<string, number> | undefined): Record<string, number> | undefined => {
  if (!stats) return undefined;
  
  const transformedStats: Record<string, number> = {};
  
  for (const [statId, value] of Object.entries(stats)) {
    const statName = PLAYER_STATS_MAP[parseInt(statId) as keyof typeof PLAYER_STATS_MAP];
    if (statName) {
      transformedStats[statName] = value;
    }
  }
  
  return Object.keys(transformedStats).length > 0 ? transformedStats : undefined;
};