...
export function LeaguePassCard({ leagueId, canManage, className }: LeaguePassCardProps) {
  ...
  const [justActivatedPass, setJustActivatedPass] = useState(false);
  ...
  // Coming back from Checkout, settle the seat or pass purchase without
  // waiting on the webhook, then clean our params out of the URL.
  // Verification is idempotent and ownership-checked server-side.
  useEffect(() => {
    if (verified.current) return;
    const url = new URL(window.location.href);
    const seatSuccess = url.searchParams.get("seat") === "success";
    const passSuccess = url.searchParams.get("pass") === "success";
    if (!seatSuccess && !passSuccess) return;
    ...
    verifyPayment({ sessionId })
      .then(() => (passSuccess ? setJustActivatedPass(true) : setJustPurchased(true)))
      .catch((err) =>
        console.error(`${passSuccess ? "League Pass" : "Seat"} verification failed:`, err)
      )
      .finally(clearParams);
  }, [verifyPayment]);
  ...
  const handleBuyPass = async () => {
    ...
    try {
      ...
      console.error("League Pass checkout error:", err);
      ...
      {canManage && (
        <div className="mt-6 flex flex-col gap-5 border-t border-bc-hairline pt-6">
          {!isActive && (
            <div className="flex flex-col gap-3">
              <Button variant="glow" size="lg" onClick={handleBuyPass} disabled={isBuyingPass}>
                ...
                {isBuyingPass ? "Opening checkout..." : `Buy League Pass · $${PASS_PRICE}`}
              </Button>
              <p className="text-[14px] leading-relaxed text-bc-text-2">
                Covers every automated story for the season, plus 100 credits for each of the{" "}
                {included} included managers.
              </p>
            </div>
          )}
          ...
          {justActivatedPass && (
            <p className="flex items-center gap-2 text-[14px] text-bc-win">
              <Check className="size-4" strokeWidth={3} aria-hidden="true" />
              League Pass active. Automated stories are on.
            </p>
          )}
